...
#or leave blank for simple default configuration:"
    ...
    #read address
    #if [ -v $address ]
    address=$(strip_quotes $(jsonc2json info.jsonc | jq ".serverSettings.address"))
    if [ $address = "" ]
    then
...
#(9) your variant"
    ...
    jsonc2json template_config_server.jsonc \
        ...
        | jq --argjson x "${server_names}" '.inbounds[1].streamSettings.realitySettings.serverNames=$x
            | .inbounds[2].streamSettings.realitySettings.serverNames=$x' \
...
no new config created fot it${normal}"
            ...
            then
                ...
                ok1=$(cat ./conf/config_client.json | jq ".outbounds[0].settings.vnext[0].users[0].id=\"${id}\" | .outbounds[0].settings.vnext[0].users[0].email=\"${email}\" | .outbounds[0].streamSettings.realitySettings.shortId=\"${short_id}\"" > ./conf/config_client_${username}.json)
...
then
    ...
    do
        ...
        then
            ...
            cp ./conf/config_server.json ./conf/tmp_server_config.json
            # update server config
            ok1=$(cat ./conf/tmp_server_config.json | jq "del(.inbounds[1].settings.clients[] | select(.email == \"${email}\")) | del(.inbounds[1].streamSettings.realitySettings.shortIds[] | select(. == ${short_id})) | del(.inbounds[3].settings.clients[] | select(.id == ${id}))" > ./conf/config_server.json)
            # then make the user (not root) an owner of a file
            [[ $SUDO_USER ]] && chown "$SUDO_USER:$SUDO_USER" ./conf/config_server.json
            if [ $command = "del" ]
            then
...
then
    ...
    then
        ...
        link="vless://${id}@${address}:443?security=tls&encryption=none&type=grpc&serviceName=${service_name}#lucynspace%20%F0%9F%AA%90%20CDN"
    fi
