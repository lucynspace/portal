...
# compute simple hash for the given string
hash () {
    str=$1
    if [ -v $str ]
    then
        echo -e "hash: no string given"
        exit 1
    else
        hash_and_check=$(echo $str | sha256sum)
        echo ${hash_and_check::8}
    fi
}
...
#or leave blank for simple default configuration:"
    ...
    server_name4cdn=$(strip_quotes $(jsonc2json info.jsonc | jq ".serverSettings.address"))
    ##
    #if [ -v $server_name4cdn ]
    #then
    #    echo -e "Enter IPv4 or IPv6 address of your xray server, or its domain name:"
    #else
    #    check_command sed "needed to make nginx's site.conf"
    #    echo -e "Enter IPv4 address of your xray server:"
    #fi
    #read address
    #if [ -v $address ]
    address=$server_name4cdn
    if [ $address = "" ]
    then
    ...
    private_key=$(strip_quotes $(jsonc2json info.jsonc | jq ".serverSettings.privateKey"))
    public_key=$(strip_quotes $(jsonc2json info.jsonc | jq ".serverSettings.publicKey"))
    if [ -z $private_key ] || [ -z $public_key ]
    then
...
#(9) your variant"
    ...
    server_names=$(jsonc2json info.jsonc | jq ".serverSettings.serverNames")
    server_names=$(echo $server_names | sed 's/\n//g') # for ok processing in jq
    fake_site=$(echo $server_names | cut -d "\"" -f 2)
    email="love@xray.com"
    #
    unsafe_mkdir conf
    unsafe_mkdir state
    ...
    jsonc2json template_config_server.jsonc \
        ...
        | jq --argjson x "${server_names}" '.inbounds[1].streamSettings.realitySettings.serverNames=$x' \
        | jq ".inbounds[1].streamSettings.realitySettings.privateKey=\"${private_key}\"
            | .inbounds[1].streamSettings.realitySettings.shortIds=[ \"${short_id}\" ]
            | .inbounds[2].settings.clients[0].id=\"${id}\"
            | .inbounds[2].streamSettings.grpcSettings.serviceName=\"${service_name}\" " \
...
but not present; to generate them, try
    ...
    if [ -v $2 ]
    then
        echo -e "${red}emails are not set${normal}
For default user, use config_client.json generated
by ${underl}install${normal} command. Otherwise use non-void emails."
        exit 1
    fi
...
no new config created fot it${normal}"
            ...
            then
                ...
                service_name=$(strip_quotes $(jq ".inbounds[2].streamSettings.grpcSettings.serviceName" ./conf/config_server.json))
                # make new user config from default user config
                ok1=$(cat ./conf/config_client.json | jq ".outbounds[0].settings.vnext[0].users[0].id=\"${id}\" | .outbounds[0].settings.vnext[0].users[0].email=\"${email}\" | .outbounds[0].streamSettings.grpcSettings.serviceName=\"${service_name}\" | .outbounds[0].streamSettings.realitySettings.shortId=\"${short_id}\"" > ./conf/config_client_${username}.json)
            ...
            user_state="
{
    ...
    \"payment\": {
        ...
        \"address\": \"\"
            ...
            "
            echo -e ${user_state} | jq '.' > ./state/users/${username}
            ...
            grpc_client_id="
          {
            ...
            ok2=$(cat ./conf/tmp_server_config.json | jq ".inbounds[1].settings.clients += [${client}] | .inbounds[1].streamSettings.realitySettings.shortIds += [\"${short_id}\"] | .inbounds[2].settings.clients += [${grpc_client_id}]" > ./conf/config_server.json)
            # then make the user (not root) an owner of a file
            [[ $SUDO_USER ]] && chown "$SUDO_USER:$SUDO_USER" ./conf/config_server.json
            if $ok1 && $ok2
            then
                ...
                echo -e "${yellow}something went wrong with email ${email}, username ${username}${normal}"
                exit 1
            fi
        fi
    done
...
sudo ./ex.sh install${normal}"
    ...
    then
        ...
        mkdir -p $dat_dir
        if cp server-rules.dat ${dat_dir}
        then
            echo -e "${green}server-rules.dat copied to ${dat_dir}${normal}"
        else
            echo -e "${red}server-rules.dat not copied to ${dat_dir}${normal}"
            exit 1
        fi
        # for cert.pem
...
or
    ...
    if [ -v $answer ] || [ ${answer::1} != "n" ]
    then
        echo -e "Enter user emails separated by spaces"
        read usernames
        add true $usernames
    fi
    #
...
then
    ...
    do
        ...
        then
            ...
            cp ./conf/config_server.json ./conf/tmp_server_config.json
            # update server config
            ok1=$(cat ./conf/tmp_server_config.json | jq "del(.inbounds[1].settings.clients[] | select(.email == \"${email}\")) | del(.inbounds[1].streamSettings.realitySettings.shortIds[] | select(. == ${short_id})) | del(.inbounds[2].settings.clients[] | select(.id == ${id}))" > ./conf/config_server.json)
            # then make the user (not root) an owner of a file
            [[ $SUDO_USER ]] && chown "$SUDO_USER:$SUDO_USER" ./conf/config_server.json
            if [ $command = "del" ]
            then
...
then
    ...
    then
        ...
        link="vless://${id}@${address}:${port}?fragment=&security=reality&encryption=none&pbk=${public_key}&fp=chrome&type=tcp&flow=xtls-rprx-vision&sni=${server_name}&sid=${short_id}#lucynspace%20%F0%9F%AA%90"
    else # grpc config
        id=$(strip_quotes $(jq ".outbounds[0].settings.vnext[0].users[0].id" $conf_file))
        address=$(strip_quotes $(jq ".outbounds[0].settings.vnext[0].address" $conf_file))
        service_name=$(strip_quotes $(jq ".outbounds[0].streamSettings.grpcSettings.serviceName" $conf_file))
        link="vless://${id}@${address}:443?security=tls&encryption=none&type=grpc&serviceName=${service_name}#lucynspace%20%F0%9F%AA%90"
    fi
...
then
    ...
    continue with name from email${normal}"
            ...
            then
                ...
                ok1=$(cat ${to}/config_client.json | jq ".outbounds[0].settings.vnext[0].users[0].id=\"${id}\" | .outbounds[0].settings.vnext[0].users[0].email=\"${email}\" | .outbounds[0].streamSettings.realitySettings.shortId=\"${short_id}\"" > ${to}/config_client_${uname_from_email}.json)
                # then make the user (not root) an owner of a file
...
Here is a list of all the commands available:
    ...
    ${bold}add ${underl}emails${normal}   add users with given emails to configs,
                    emails should by separated by spaces
    ${bold}del ${underl}emails${normal}   delete users with given emails from configs
    ${bold}suspend ${underl}usernames${normal} delete users with given emails from the server config
                      but don't delete the user configs
    ${bold}resume ${underl}emails${normal}  add users from suspended configs to the server config
    ...
    ${bold}hash ${underl}\"email\"${normal}      show simple hash sum for given email
